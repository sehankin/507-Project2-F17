...
# 1. a __str__() method, that returns a string of the form 'TITLE by AUTHOR'
# 2. a __repr__() method, which returns "ITUNES MEDIA: <itunes id>"
#   with the iTunes id number for the piece of media (e.g. the track)
#   only in place of "<itunes id>"
# 3. a __len__(), which, for the Media class, returns 0 no matter what.
#   (The length of an audiobook might mean something different from the
#   length of a song, depending on how you want to define them!)
# 4. a __contains__() method (for the in operator) which takes one additional
#   input, as all contains methods must, which should always be a string,
#   and checks to see if the string input to this contains method is INSIDE
...
# Now, you'll define 2 more different classes, each of which
#   *inherit from* class Media:
# 1. class Song
# 2. class Movie

# In the class definitions, you can assume a programmer would pass to each
# class's constructor only a dictionary that represented the correct
# media type (song, movie).
...
# 1. album (the album title)
# 2. track_number (the number representing its track number on the album)
# 3. genre (the primary genre name from the data iTunes gives you)

# Should have the len method overridden to return the number of seconds
...
# Should have the following additional instance variables:
# 1. rating (the content advisory rating, from the data)
# 2. genre
# 3. description (if none, the value of this instance variable should be None)
#   NOTE that this might cause some string encoding problems for you to debug!
# HINT: Check out the Unicode sub-section of the textbook! This is a common
# type of Python debugging you'll encounter with real data...but using
# the right small amount of code to fix it will solve all your problems.

# Should have the len method overridden to return the number of minutes in the
# movie (HINT: The data returns the number of milliseconds in the movie...
# how can you convert that to minutes?)

# Should have an additional method called title_words_num that returns an
# integer representing the number of words in the movie description.
